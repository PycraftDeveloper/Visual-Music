...
class Water(moderngl_window.WindowConfig):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.drop = np.array([[0.0, 0.0, 1/6, 1/5, 1/4, 1/5, 1/6, 0.0, 0.0],
        ...
        self.drops_texture.write(self.drop.astype('f4').tobytes())
    ...
    def render(self, time, frame_time):
        ...
        '''
        if volume_norm+3 >= self.max:
            self.ctx.enable(moderngl.BLEND)
            self.ctx.blend_func = moderngl.ONE, moderngl.ONE
            self.drops_texture.use()
            self.mouse_pos
            self.drop_program['pos'].value = (0.5, 0.5)
        ...
        if volume_norm <= self.max/4:
            self.ctx.enable(moderngl.BLEND)
            self.ctx.blend_func = moderngl.ONE, moderngl.ONE
            self.drops_texture.use()
            self.mouse_pos
            self.drop_program['pos'].value = (-(volume_norm/self.max), -(volume_norm/self.max))
            self.sprite.render(self.drop_program)
            self.ctx.disable(moderngl.BLEND)
            ...
            self.drops_texture.use()
            self.mouse_pos
            self.drop_program['pos'].value = ((volume_norm/self.max)/1, (volume_norm/self.max)/1)
            self.sprite.render(self.drop_program)
            self.ctx.disable(moderngl.BLEND)
            ...
            self.drops_texture.use()
            self.mouse_pos
            self.drop_program['pos'].value = (-(volume_norm/self.max), (volume_norm/self.max)/1)
            self.sprite.render(self.drop_program)
            self.ctx.disable(moderngl.BLEND)
            ...
            self.drops_texture.use()
            self.mouse_pos
            self.drop_program['pos'].value = ((((volume_norm/self.max))/1), -(volume_norm/self.max))
            self.sprite.render(self.drop_program)
            self.ctx.disable(moderngl.BLEND)
        else:
            if volume_norm >= (self.max/4)*3:
                self.ctx.enable(moderngl.BLEND)
                self.ctx.blend_func = moderngl.ONE, moderngl.ONE
                self.drops_texture.use()
                self.mouse_pos
        ...
        if volume_norm >= (self.max/4) and volume_norm <= (self.max/4)*3.5: # 5-7 8
            self.ctx.enable(moderngl.BLEND)
            self.ctx.blend_func = moderngl.ONE, moderngl.ONE
            self.drops_texture.use()
            self.mouse_pos
            self.drop_program['pos'].value = (0, 0)
            self.sprite.render(self.drop_program)
            self.ctx.disable(moderngl.BLEND)
